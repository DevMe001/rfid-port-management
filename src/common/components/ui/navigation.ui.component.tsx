		...
		const onTabToggle = useCallback(
			(label: string) => {
				...
				if (label.toLowerCase().includes('schedule')) {
					...
					if (label.toLowerCase().includes('wallet')) {
						dispatch({ type: 'wallet' });
					}
			},
			[booking, settings, schedule,wallet],
		);
							...
							<>
								<li key={i} onClick={() => onTabToggle(list.label)}>
									<a href='#' className='flex gap-5 items-center pr-5'>
										...
										<RenderDropdownIcon dropUpShow={(list.wallet as boolean) && wallet} dropDownShow={(list.wallet as boolean) && !wallet} />
									</a>
								</li>
