...
const PopupModal: React.FC<ModalProps> = ({ children, maxWidth, onClose }) => {
	...
	return (
		<div
			onClick={onOpen}
			className={clsx('absolute top-0 left-0 right-0 bottom-0 bg-[rgba(0,0,0,0.6)] z-10 w-screen h-screen', {
				'max-w-[90rem]': isUndefined(maxWidth),
			})}
			ref={modalRef}
		>
			<div className='flex flex-col justify-center items-center h-full'>
				{/*  iwant this not hied when someone clicn in this part of box */}
				<div onClick={handleContentClick} className={clsx('bg-white w-auto h-2/2 rounded-md animate-fade relative')}>
					<p className='flex w-auto justify-end items-center font-bold text-navy pr-5 pt-5 text-3xl'>
						<span onClick={onClose ?? onOpen}>&times;</span>
					</p>
